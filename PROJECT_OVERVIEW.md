...
│   │   └── checkin/            # Kiosk UI: family-lookup (QR+phone), child-selection (multi-select cards), allergy-confirm, checkin-success (security code display), numeric-keypad, child-card, room-picker-modal, room-child-card, allergy-detail-modal, visitor-registration (first-time family self-registration), checkout-entry, checkout-success, kiosk-install-prompt (PWA install banner). Wave 9 P0-2 additions: photo-capture (greenfield webcam + file fallback for pickup-contact photos), photo-thumbnail (signed-URL fetcher for admin display of check-in photos), authorized-pickup-panel (per-child management UI on the household detail page)
│   └── lib/
