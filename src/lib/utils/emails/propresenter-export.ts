  ...
  const html = `
    <div style="font-family: 'DM Sans', sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px;">
      <div style="margin-bottom: 24px;">
        <p style="color: #2D3047; font-size: 16px; margin: 0;">Hi ${escapeHtml(recipientName)},</p>
      </div>
      <div style="background: #FBF7F0; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
        <p style="color: #2D3047; font-size: 18px; font-weight: 600; margin: 0 0 4px 0;">${escapeHtml(title)}</p>
      ...
      <p style="color: #9CA3AF; font-size: 12px; margin: 0;">
        Sent by VolunteerCal on behalf of ${escapeHtml(churchName)}
      </p>
    </div>
  `.trim();
