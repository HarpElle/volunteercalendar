...
interface ShareMenuProps {
  ...
  onEmailInvite?: () => void;
  copied?: boolean;
  /** When true, short link already exists for this target */
  hasShortLink?: boolean;
  /** The resolved short link URL (e.g., https://volunteercal.com/s/easter) */
  shortLinkUrl?: string;
...
const btnClass =
  "flex w-full items-center gap-2.5 px-3 py-2 text-left text-xs text-vc-text hover:bg-vc-bg-warm transition-colors";
...
export function ShareMenu({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-full z-20 mt-1 w-52 rounded-xl border border-vc-border-light bg-white py-1 shadow-lg">
          {/* 1. Short link (create or edit) — always first with premium badge */}
          {onCreateShortLink && (
            <button
          ...
          <button
            onClick={() => {
              if (hasShortLink && shortLinkUrl && onCopyShortLink) {
                onCopyShortLink();
              } else {
                onCopyLink();
              }
          ...
          >
            ...
            {copied ? "Copied!" : hasShortLink && shortLinkUrl ? "Copy short link" : "Copy link"}
          </button>
