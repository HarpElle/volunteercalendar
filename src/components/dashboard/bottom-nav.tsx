...
/**
 * Volunteer-shell bottom tabs. A check-in volunteer / check-in manager who
 * isn't a scheduler gets a dedicated Check-In tab (they don't get the admin
 * shell's More menu) so they can still reach the classroom views (Codex P3-8).
 */
function getVolunteerTabs(canShowCheckin: boolean): TabItem[] {
  const tabs: TabItem[] = [
    {
      label: "Schedule",
  ...
  if (canShowCheckin) tabs.push(CHECKIN_TAB);
  tabs.push({
    label: "Account",
...
export function BottomNav({
  ...
  const tabs: Tab[] = isAdmin
    ? getAdminTabs(worshipEnabled, canShowCheckin)
    : getVolunteerTabs(canShowCheckin);
    ...
    >
      <div className="flex">
        {tabs.map((tab) => {
          ...
          const showBadge =
            tab.href === "/dashboard/inbox" && hasUnreadNotifications;
