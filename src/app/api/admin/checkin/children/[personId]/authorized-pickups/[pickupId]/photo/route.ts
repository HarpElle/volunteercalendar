 ...
 *
 * Wave 9 P0-2 sub-PR C. Multipart upload writes to Storage via
 * `uploadCheckInPhoto()`; the resulting storage path is persisted as
 * `photo_url` on the contact. Reads of the photo are NEVER unsigned —
 * the UI calls /api/admin/checkin/photo?path=... for a short-TTL
...
export async function POST(
  ...
  try {
    const { personId, pickupId } = await params;
