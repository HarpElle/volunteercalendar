...
## Phase Roadmap
See `PROJECT_OVERVIEW.md` for complete phase history (Phases 1–32 + Expansion Phases 4–11 + Phase G + Part 3 complete). All expansion phases are done: SongSelect integration, Stage Sync, ProPresenter export, song usage reports, platform admin tier override, ministry templates, volunteer archive/status system, ChordPro/PDF import with chord chart viewer system, native children's check-in with kiosk UI, label printing, companion print service, room & resource scheduling with booking wizard, conflict detection, recurring reservations, approval workflow, iCal feeds, wall-mounted display signage, and WorshipTools UX improvements (service plan editor with header items + inline notes, volunteer availability indicators, self-service availability page, batched notification emails, ministry group collapse/expand, multi-service compare view).
