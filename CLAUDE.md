...
## Phase Roadmap
See `PROJECT_OVERVIEW.md` for complete phase history (Phases 1–32 + Expansion Phases 4–11 + Phase G complete). All expansion phases are done: SongSelect integration, Stage Sync, ProPresenter export, song usage reports, platform admin tier override, ministry templates, volunteer archive/status system, ChordPro/PDF import with chord chart viewer system, native children's check-in with kiosk UI, label printing, companion print service, and room & resource scheduling with booking wizard, conflict detection, recurring reservations, approval workflow, iCal feeds, and wall-mounted display signage.
